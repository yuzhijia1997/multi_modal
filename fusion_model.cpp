...
bool FusionModel::IsExistGallery()
{
...
bool FusionModel::GetGalleryFiles(std::vector<std::vector<std::string>>& files)
{
	...
	do
	{
		if (file_info.attrib == _A_SUBDIR && strcmp(file_info.name, ".") && strcmp(file_info.name, "..")) {
			...
			std::string path;
			path = dirname + "\\" + file_info.name + "\\face";
			std::vector<std::string> facePathList;
			cv::glob(path, facePathList);
			path = dirname + "\\" + file_info.name + "\\iris";
			std::vector<std::string> irisPathList;
			cv::glob(path, irisPathList);
			path = dirname + "\\" + file_info.name + "\\fingerprint";
			std::vector<std::string> fpPathList;
			cv::glob(path, fpPathList);
			int image_num = std::min(fpPathList.size(), std::min(facePathList.size(), irisPathList.size()));
			for (int i = 0;i < image_num; ++i) {
				files.push_back({ file_info.name,facePathList[i],irisPathList[i],fpPathList[i] });
			}
		}
...
std::string FusionModel::Accelerate()
{
	...
	if (record.is_open()) {
		...
		while (record >> model && !record.eof()) {
			...
			if (model == mode && gallery == get_gallery_dir()) {
				record.close();
				return file_path;
			}
		}
	...
	for (int i = 0;i < person_num; i = i + 64) {
		std::vector<at::Tensor> faceinputsVector,irisinputsVector,fpinputsVector;
		int current_num = std::min(i + 64, person_num) - i;
		gfeats_file.write((char*)&current_num, sizeof(int));
		int a = (128 + 1024 * 3) * current_num;
		gfeats_file.write((char*)&a, sizeof(int));
		for (int j = i;j < i + current_num;++j) {
			at::Tensor inputTensor;

			GetImageInput(gallery_files[j][1], inputTensor);
			faceinputsVector.push_back(inputTensor.clone());
			GetImageInput(gallery_files[j][2], inputTensor);
			irisinputsVector.push_back(inputTensor.clone());
			GetImageInput(gallery_files[j][3], inputTensor);
			fpinputsVector.push_back(inputTensor.clone());

			char person[128], image_path[1024];
			strcpy_s(person, gallery_files[j][0].c_str());
			gfeats_file.write(person, 128);
			for (int k = 1;k < 4;++k) {
				strcpy_s(image_path, gallery_files[j][k].c_str());
				gfeats_file.write(image_path, 1024);
			}
		}
		auto faceinputsTensor = torch::cat(faceinputsVector, 0);
		auto irisinputsTensor = torch::cat(irisinputsVector, 0);
		auto fpinputsTensor = torch::cat(fpinputsVector, 0);
		auto inputs = torch::stack({ faceinputsTensor,irisinputsTensor,fpinputsTensor }, 0);
	...
	record1 << mode << " " << get_gallery_dir() << " " << file_path << std::endl;
	record1.close();
	return file_path;
}

void FusionModel::ComputeDis(const std::vector<std::string>& imageNames, std::vector<float>& distance, std::vector<std::vector<std::string>>& gallery_files)
{
	...
	auto inputs = torch::stack(inputsVector, 0);
	inputs = inputs.to(device);
	...
	if (get_is_accelerate()) {
		...
		while (gfeats_file.read((char*)&current_num, sizeof(int)) && !gfeats_file.eof()) {
			...
			for (int i = 0;i < current_num;++i) {
				char person[128], face_path[1024], iris_path[1024], fp_path[1024];
				gfeats_file.read(person, 128);
				gfeats_file.read(face_path, 1024);
				gfeats_file.read(iris_path, 1024);
				gfeats_file.read(fp_path, 1024);
				gallery_files.push_back({ person,face_path,iris_path,fp_path });
			}
			gfeats_file.read((char*)&size, sizeof(int));
	...
	else
	{
		...
		for (int i = 0;i < gallery_files.size(); i = i + 64) {
			...
			auto irisinputsTensor = torch::cat(irisinputsVector, 0);
			auto fpinputsTensor = torch::cat(fpinputsVector, 0);
			inputs = torch::stack({ faceinputsTensor,irisinputsTensor,fpinputsTensor }, 0);
			inputs = inputs.to(device);
...
std::vector<std::vector<std::string>> FusionModel::GetMatchResult(const std::vector<std::string>& imageNames)
{
