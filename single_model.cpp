...
bool SingleModel::IsExistGallery()
{
...
bool SingleModel::GetGalleryFiles(std::vector<std::vector<std::string>>& files)
{
	...
	do
	{
		if (file_info.attrib == _A_SUBDIR && strcmp(file_info.name, ".") && strcmp(file_info.name, "..")) {
			std::string image_path = dirname + "\\" + file_info.name + "\\" + modal;
			std::vector<std::string> imagePathList;
			cv::glob(image_path, imagePathList);
...
std::string SingleModel::Accelerate()
{
	...
	if (record.is_open()) {
		...
		while (record >> model && !record.eof()) {
			...
			if (model == modal && gallery == get_gallery_dir()) {
				record.close();
				return file_path;
			}
		}
	...
	record1 << modal << " " << get_gallery_dir() << " " << file_path << std::endl;
	record1.close();
	return file_path;
}

void SingleModel::ComputeDis(const std::string& imageName, std::vector<float>& distance, std::vector<std::vector<std::string>>& gallery_files)
{
	at::Tensor inputs;
...
std::vector<std::vector<std::string>> SingleModel::GetMatchResult(const std::string& imageName)
{
